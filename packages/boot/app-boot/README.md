...
Before mounting profile rows, the `dsh` launcher computes one immutable package-resolution generation from the installation and ordered bundle dependency graphs. The CLI defaults to link mode (materialized fallback links), except in packaged executables where runtime mode is forced: it installs the generation through Node's ESM and CommonJS resolvers without creating fallback links. Callers of `runProfile` may explicitly select link mode, dual mode, or runtime mode. Packaged executables and the Electron Host always use runtime mode.
