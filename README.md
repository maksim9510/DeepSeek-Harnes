...
Команда `pnpm run dev:web` собирает, запускает и пересобирает клиентские бандлы при правках исходников в одном терминале; `make help` показывает соответствующие Make-цели для Web и Desktop — полную таблицу команд приложения ведёт раздел руководства.
